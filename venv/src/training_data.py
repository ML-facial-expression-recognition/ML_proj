...
def lets_train(x, y, classifier, strategy):
    # decomposition PCA
    pca = decomposition.PCA(n_components=24).fit(x)
    ...
    if strategy == 'ova':
        if classifier == 'svm':
            ...
            clf = multiclass.OneVsRestClassifier(ensemble.BaggingClassifier(tree.DecisionTreeClassifier(max_depth=5),
                                                                            n_estimators=200)).fit(newx, y)
        elif classifier == 'logistic':
            clf = multiclass.OneVsRestClassifier(LogisticRegression(C=1e3, penalty='l2', solver='newton-cg')).fit(newx,
                                                                                                                  y)
        elif classifier == 'mlp':
            clf = multiclass.OneVsRestClassifier(neural_network.MLPClassifier(solver='sgd', activation='relu',
                                                                              max_iter=100, alpha=1e-5,
                                                                              hidden_layer_sizes=(300, 400, 200, 200),
        ...
        if classifier == 'svm':
            clf = multiclass.OneVsOneClassifier(SVC(C=1e1, gamma='auto')).fit(newx, y)
        elif classifier == 'decisionTree':
            clf = multiclass.OneVsOneClassifier(
                ensemble.BaggingClassifier(tree.DecisionTreeClassifier(max_depth=4), n_estimators=200)).fit(newx, y)
        elif classifier == 'logistic':
            clf = multiclass.OneVsOneClassifier(LogisticRegression(C=1e8, penalty='l2', solver='newton-cg')).fit(newx,
                                                                                                                 y)
        elif classifier == 'mlp':
            clf = multiclass.OneVsRestClassifier(neural_network.MLPClassifier(solver='sgd', activation='logistic',
                                                                              max_iter=200, alpha=1e-5,
...
if __name__ == '__main__':
    ...
    # feature_path = '../img/race/feature.npy'
    training_data(feature_path,'ova','logistic')
